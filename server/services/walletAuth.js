...
const NONCE_TTL_MS    = 5 * 60 * 1000;          // 5 minutes to sign challenge
const SESSION_TTL_MS  = 24 * 60 * 60 * 1000;    // 24 hours active dashboard session
...
/**
 * Constant-time utility to prevent timing leaks during token validation.
 */
 ...
 * Returns:
 *   {
 *     token,    // opaque envelope, must be returned with the signature
...
export function requireWalletSession(req, res, next) {
  ...
  const opened = openEnvelope(token);
  if (!opened.ok) return res.status(401).json({ error: opened.ok ? undefined : opened.error });
