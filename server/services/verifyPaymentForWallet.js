 ...
 *
 * Verifies on-chain USDC payments made to any arbitrary publisher wallet address.
 * Enables multi-tenant paywall facilitation.
 *
 * Consolidated Cache Integration (Option 2):
 * Uses `isTxCachedGlobal` and `cacheTxGlobal` from `wallets.js` to prevent double spend
 * replay attacks, ensuring transaction cache entries are mapped to the correct wallet.
 *
 * Mock Mode Integration:
 * If MOCK_VERIFICATION is enabled or if the signature starts with "mock_", the verification
 * bypasses Solana RPC nodes and returns a success response, caching the mock signature.
 */
...
export function buildWalletContext({ walletAddress, network, usdcMint }) {
  ...
  // We reuse connections via a cache map to minimize socket allocations.
  const rpcUrl = rpcUrlFor(net);
  let connection = null;
  
  if (!isMockMode) {
    connection = rpcCache.get(rpcUrl);
    if (!connection) {
...
/**
 * Public wrapper for verifyPaymentForWallet.
 */
...
async function verifyInner({
  ...
  if (isMockMode || signature.startsWith('mock_')) {
    ...
    return {
      verified: true,
      received: Number(requiredMicroUsdc),
  ...
  if (!ctx.connection) {
    return { verified: false, error: "Server misconfigured. Missing SOLANA_RPC_URL for live network verification." };
  }
  ...
  if (!tx && payment.payload.serializedTransaction) {
    try {
      ...
      // Ignore raw transaction broadcast errors; fall through to fetch retry
    }
