 ...
 *
 * Exposes endpoints for SDK challenges, dynamic payment verification,
 * SIWS login authentication, and dashboard analytics metrics.
 *
 * Caching & UX Optimization:
 * - Implements dynamic Associated Token Account (ATA) auto-creation via a fee-payer keypair.
 * - Pools and caches ATA verification promises (`ataExistsCache`, `ataEnsureInFlight`) to eliminate redundant RPC lookups.
 */
...
const router = Router();
const ATA_EXISTS_CACHE_TTL_MS = 10 * 60 * 1000; // 10 minutes cache
const ataExistsCache = new Map();
const ataEnsureInFlight = new Map();

/**
 * Parsers a base58 or JSON-array formatted secret key for transaction fee sponsorships.
 */
...
/**
 * Ensures that the receiving publisher's USDC Associated Token Account exists.
 * If not, automatically creates and pays the rent fee for it on-chain to maximize onboarding UX.
 */
async function ensureTreasuryAta({ ctx, walletAddress }) {
  ...
  const promise = (async () => {
    // If in Mock Mode, we bypass blockchain lookups
 ...
 * GET /v1/dashboard
 *
 * Auth: Bearer <session> or X-PAYWALL-SESSION header
 */
